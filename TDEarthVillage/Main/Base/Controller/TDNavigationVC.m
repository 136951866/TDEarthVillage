...
#define kTOP_VIEW kHankKeyWindow.rootViewController.view
const float kMargin = 60;

@interface TDNavigationVC ()<UINavigationControllerDelegate,UIGestureRecognizerDelegate>{
    CGPoint startTouch;
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if (self.screenShotsList.count == 0) {
        UIImage *capturedImage = [TDNavigationVC capture];
        if (capturedImage) {
...
- (void)setNormalState{
    ...
    //navigationBar.translucent = YES;
    navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName: HANKColor(48, 48, 48),
...
+ (UIImage *)capture{
    UIView *aView = kHankCurrentWindow;
    if (!aView) {
        return nil;
    }
...
- (void)paningGestureReceive:(UIPanGestureRecognizer *)recoginzer{
    ...
    CGPoint touchPoint = [recoginzer locationInView:kHankKeyWindow];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if (self.viewControllers.count > 0) { // 这时push进来的控制器viewController，不是第一个子控制器（不是根控制器）
        ...
        viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(popBackAction:) image:@"common_back" highImage:@"common_back"];
    }
